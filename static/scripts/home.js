...
function increase(name) {
  ...
  price.innerText =
    "💰" +
    (parseInt(price.innerText.replace("💰", "")) / amount) * (amount + 1);
}
...
function buy(stock_owner) {
  ...
  window.location.href = `https://stocksim.onrender.com/buy?user=${stock_owner}&amount=${amount}&redirect=${window.location.href}`;
}
...
setInterval(async () => {
  const settings = {
    async: true,
    crossDomain: true,
    url: "https://stocksim.onrender.com/stock_api",
    method: "GET",
  };
